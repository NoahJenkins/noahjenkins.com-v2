...
If the pull request head changes after CI or at any later write boundary, the evaluator treats the run as stale and exits without changing labels, reviews, or native auto-merge. A zero-permission `pull_request_target` signal from the trusted base workflow cancels the in-progress evaluator when Dependabot synchronizes the head; opens, reopens, closes, or converts the pull request to draft; becomes ready for review; or changes the base branch. Title and body edits do not cancel. Synchronize, open, reopen, and ready-for-review events start replacement CI; close, draft conversion, and a base change stop unsafe evaluation. The write-capable job remains restricted to `workflow_run`. A rollback must re-confirm that the evaluator still owns the validated head before it disables auto-merge.
